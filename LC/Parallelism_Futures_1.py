...
# futures : 비동기 실행을 위한 API를 고수준으로 작성 -> 사용하기 쉽도록 개선하기 위해 나옴
# concurrent.Futures
...
# GIL(Global interpreter Lock): 여러 개의 작업을 처리할 때 전체가 Lock이 걸리는 현상
# 두 개 이상의 스레드가 동시에 실행 될 때 하나의 자원을 액세스 하는 경우 -> 문제점을 방지하기 위해
# GIL 실행 , 즉 리소스 전체에 Lock이 걸림 -> Context Switch(문맥 교환)
